...
- Binary Tree Game:
    ...
    - After that a text box appears requesting a number to generate the nodes of the tree; the player should write down a number from 1 to 32 prefereably in order not to slow the game or crash it; then the nodes are                generated with an equal number of balls for each node specifically; the player should grab each player by lmb and either place it directly or throw it with rmb; the player should pay attention to the tree rules to add        the correct number in the middle and the lesser one on the left and greater one on the right while filling the smallest level before moving to the next one.
